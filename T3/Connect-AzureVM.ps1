...
Add-AzureRMAccount -ServicePrincipal -Tenant $SPConnection.TenantID `
-ApplicationId $SPConnection.ApplicationID -CertificateThumbprint $SPConnection.CertificateThumbprint | Write-Verbose


	# Select the Azure subscription we will be working against
    Select-AzureRMSubscription -SubscriptionName $AzureSubscriptionName | Write-Verbose

    InlineScript { 
        # Get the Azure certificate for remoting into this VM
        $winRMCert = (Get-AzureRMVM -ResourceGroupName $ResourceGroupName -Name $Using:VMName | select -ExpandProperty vm).DefaultWinRMCertificateThumbprint   
        $AzureX509cert = Get-AzureCertificate -ResourceGroupName $ResourceGroupName -Thumbprint $winRMCert -ThumbprintAlgorithm sha1
		...
		Get-AzureWinRMUri -ResourceGroupName $ResourceGroupName -Name $Using:VMName     
    }
}